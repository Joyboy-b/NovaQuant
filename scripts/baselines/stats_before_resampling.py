...
def bootstrap_mean_ci(x: List[float], n: int = 2000, alpha: float = 0.05, seed: int | None = None) -> Dict[str, float]:
    ...
    # Bound temporary arrays rather than allocating n * len(x) at once.
    means = np.concatenate([rng.choice(arr, size=(min(32,n-i),arr.size),replace=True).mean(axis=1)
...
def permutation_test_mean_gt_zero(x: List[float], n: int = 5000, seed: int | None = None) -> Dict[str, float]:
    ...
    # sign flip
    perm_means = np.concatenate([(arr*rng.choice([-1.0,1.0],size=(min(32,n-i),arr.size))).mean(axis=1)
                                for i in range(0,n,32)])
